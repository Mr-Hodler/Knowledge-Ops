...
**Due diligence and data room** (investor-ops) use a jurisdiction-neutral taxonomy, with the **reference standard selected at runtime from your configured jurisdiction**, because investor expectations for a seed data room differ materially by market and preparing to the wrong one makes a founder look unprepared to their actual investors. The worked example shipped with the skill is the Swiss one, derived from the [SICTIC / Swiss ICT Investor Club](https://www.sictic.ch/) due diligence checklist and the Swiss Angel Investor Handbook, [Innosuisse](https://www.innosuisse.admin.ch/) screening criteria, a Startup Board Academy governance due diligence list, and an M&A acquisition request list (corporate plus engineering and infrastructure). These map to the canonical data room taxonomy and the per-stage profiles (seed to M&A).
