...
| | **Competition & Grant Submission** | "prepare our application for <competition / grant / accelerator>", "what does this call require", "are we eligible", "assemble the annexes" |
...
- **Competition & Grant Submission.** A different examiner, not a lighter version of Fundraise Prep. A jury or a grant evaluator receives **one self-contained document plus named annexes**, at a **hard deadline**, scores it against a **published weighted rubric**, and compares it **against the other submissions in the round**. Nothing is pulled from a room, and nothing outside the document exists. So: retrieve the call's current criteria, weights, eligibility rules and annex list at runtime with an absolute date (never assume a rubric, forms change between cycles); **check eligibility before writing anything**, because a single rule invalidates the whole submission; order the sections to match the criteria and answer each in the programme's own vocabulary; allocate effort to weight; answer the sector overlay, not only the common core; and assemble every mandated annex, since a missing one is usually a formal rejection rather than a lost point. Assembles from Founder-OS, does not author. File the decision letter, approval or rejection, next to the submission: a rejection names the criteria you failed and is the best input to the next attempt. See `references/checklists.md`.
